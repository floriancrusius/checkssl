...
/**
 * Parse a date in German (dd.mm.yyyy) or American (mm/dd/yyyy) format
 * @param {string} dateString - Date string in format dd.mm.yyyy or mm/dd/yyyy
...
const parseDate = (dateString) => {
  ...
  // Check for American format (mm/dd/yyyy)
    ...
    if (parts.length !== 3) {
      throw new Error(
        `Invalid date format: ${dateString}. Expected mm/dd/yyyy`,
      );
    }
    [month, day, year] = parts.map(Number);
  } else {
    throw new Error(
      `Invalid date format: ${dateString}. Expected dd.mm.yyyy or mm/dd/yyyy`,
    );
  }
 ...
 * @example
 * formatResultsCSV([{ domain: 'example.com', result: '01.01.2025' }]);
 * // 'Domain,Expiration\nexample.com,01.01.2025'
 */
const formatResultsCSV = (results) => {
  ...
  const rows = results.map(({ domain, result }) => {
    // Escape domain and result for CSV (handle commas, quotes)
    const escapedDomain = `"${String(domain).replace(/"/g, '""')}"`;
    const escapedResult = `"${String(result).replace(/"/g, '""')}"`;
    return `${escapedDomain},${escapedResult}`;
  });
 ...
 * @example
 * formatResultsJSON([{ domain: 'example.com', result: '01.01.2025' }]);
...
const formatResultsJSON = (results) => {
  ...
  const jsonResults = results.map(({ domain, result }) => ({
    domain: String(domain),
    expiration: String(result),
  }));
...
const sendHelp = () => {
  ...
  console.log('  --format <type>  Output format: table (default), csv, json');
